...
# Serialize state transitions for each MM deal. This prevents two users
# clicking role/confirmation controls at nearly the same time from racing.
_mm_flow_locks = {}


def mm_flow_lock(deal_id):
    return _mm_flow_locks.setdefault(str(deal_id), asyncio.Lock())
...
async def delayed_mm_ping(channel, content, *, users=False, roles=False, delay=1.5):
    ...
    try:
        await channel.send(
            content=content,
            allowed_mentions=discord.AllowedMentions(users=users, roles=roles)
        )
    except Exception as e:
    ...
    async def callback(self, interaction):
        ...
        async with mm_flow_lock(self.deal_id):
            ...
            if actor_id not in participant_ids:
                await interaction.followup.send("❌ Only ticket participants can select a role.", ephemeral=True)
                return
            roles = {str(uid): value for uid, value in deal.get("roles", {}).items()}
    ...
    async def callback(self, interaction):
        ...
        async with mm_flow_lock(self.deal_id):
            ...
            if not deal or str(interaction.user.id) not in {str(uid) for uid in deal.get("participants", [])}:
                await interaction.followup.send("❌ Only the deal participants can reset roles.", ephemeral=True)
                return
            deal["roles"] = {}
    ...
    async def callback(self, interaction):
        ...
        async with mm_flow_lock(self.deal_id):
            ...
            if not deal or actor_id not in participant_ids:
                await interaction.followup.send("❌ Only the two participants can confirm the roles.", ephemeral=True)
                return
            if not self.correct:
                deal["roles"] = {}
                deal["role_confirmed"] = {str(uid): False for uid in deal.get("participants", [])}
            ...
            if participants and all(confirmed.get(uid, False) for uid in participants):
                buyer_id = next((str(uid) for uid, role in deal.get("roles", {}).items() if role == "buyer"), None)
                if not buyer_id:
                    await interaction.followup.send("❌ A Buyer must be selected before entering the offer.", ephemeral=True)
                    return
...
class CancelDealAmountButton(discord.ui.Button):
    def __init__(self, deal_id):
        super().__init__(label="Cancel Deal Amount", style=discord.ButtonStyle.danger, custom_id=f"mm:deal_cancel:{deal_id}", row=2)
        self.deal_id = deal_id
    ...
    async def callback(self, interaction):
        ...
        async with mm_flow_lock(self.deal_id):
            deal = _mm_deals.get(self.deal_id)
            if not deal or str(interaction.user.id) not in {str(uid) for uid in deal.get("participants", [])}:
                await interaction.followup.send("❌ Only the deal participants can cancel this amount.", ephemeral=True)
                return
            deal["price"] = None
...
class USDConfirmButton(discord.ui.Button):
    def __init__(self, deal_id):
        super().__init__(label="Confirm Deal", style=discord.ButtonStyle.success, custom_id=f"mm:deal_confirm:{deal_id}", row=0)
        self.deal_id = deal_id
...
class MMConfirmButton(discord.ui.Button):
    def __init__(self, deal_id):
        super().__init__(label="Confirm Deal", style=discord.ButtonStyle.success, custom_id=f"mm:confirm:{deal_id}", row=0)
        self.deal_id = deal_id
    ...
    async def callback(self, interaction):
        ...
        async with mm_flow_lock(self.deal_id):
            ...
            participants = [str(uid) for uid in deal.get("participants", [])]
            if participants and all(confirmed.get(uid, False) for uid in participants):
                raw = str(deal.get("price") or "")
                match = re.search(r"(?<!\d)(\d+(?:\.\d+)?)", raw)
                if not match:
                    await interaction.message.edit(embed=mm_deal_embed(deal), view=DealConfirmView(self.deal_id))
                    await interaction.followup.send("❌ Add an integer deal value before confirming so MM routing can be selected.", ephemeral=True)
                    return
                deal["usd_routing_value"] = int(float(match.group(1)))
                deal["state"] = "routing_mm"
                save_mm_deals(_mm_deals)
